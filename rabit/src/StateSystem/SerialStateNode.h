...
#ifndef SerialStateNode_H_
#define SerialStateNode_H_

#include "StateNode.h"

namespace DTX_StateSystem
{
    //A State that supports runing a set of substates in Serial.
    //Each substate should be designed to go from a reset to
    //complete state.  Each substate is run in order from it
    //its reset state to its complete state.
    ...
    //    Error:  means one or more substates is in an error state.
    //            The set of states stop after the first error.
	class SerialStateNode : public StateNode
	{
	public:
